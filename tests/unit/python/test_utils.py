import pytest
from app.routes import validate_analysis_params, calculate_avg_rent, calculate_avg_cost_for_square, find_top_zones
from app.routes import MIN_RADIUS, MAX_RADIUS, MIN_RENT, MAX_RENT, MIN_COMPETITORS, MAX_COMPETITORS, MIN_AREA_COUNT, MAX_AREA_COUNT

# Тесты для validate_analysis_params
...
def test_validate_analysis_params_out_of_range_radius():
    args = {
        "city_id": "1",
        "category_id": "2",
        "radius": str(MAX_RADIUS + 1), # invalid
        "rent": "100000",
        "competitors": "5",
        "area_count": "10",
    }
    ...
    assert errors is not None
    assert "radius" in errors
    assert errors["radius"] == f"Радиус должен быть между {MIN_RADIUS} и {MAX_RADIUS} км."
    
# Тесты для calculate_avg_rent
...
# Тесты для calculate_avg_cost_for_square
...
def test_calculate_avg_cost_for_square_valid():
    places = [
        {"price": 1000, "total_area": 10}, # 100
        {"price": 2500, "total_area": 20}, # 125
        {"price": 1500, "total_area": 15}  # 100
    ] # Avg = (100+125+100)/3 = 108.33 -> 108
...
def test_calculate_avg_cost_for_square_with_zero_area():
    places = [
        ...
        {"price": 2000, "total_area": 0}, # Будет проигнорировано
