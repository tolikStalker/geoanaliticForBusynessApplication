...
def test_register_duplicate_username(client):
    response1 = client.post('/register', json={'username': 'user1', 'password': 'pw1'})
    assert response1.status_code == 200

    response_logout = client.post('/logout')
    assert response_logout.status_code == 200

    response2 = client.post('/register', json={'username': 'user1', 'password': 'pw2'})
    assert response2.status_code == 400
    assert response2.json['error'] == 'User already exists'


def test_register_when_already_logged_in(logged_in_client):
    response = logged_in_client.post('/register', json={'username': 'anotheruser', 'password': 'pw'})
    assert response.status_code == 400
    assert response.json['error'] == 'Already logged in'

def test_login_correct_credentials(client,test_user):
    response = client.post('/login', json={'username': test_user.username, 'password': 'password'})
    assert response.status_code == 200
    ...
    with client.session_transaction() as sess:
        ...
        assert 'last_activity' in sess # Проверяем, что before_request добавляет это

def test_login_incorrect_password(client):
    response1 = client.post('/register', json={'username': 'user2', 'password': 'pw2'})
    assert response1.status_code == 200
...
def test_session_timeout(app, client, test_user):
    app.config['PERMANENT_SESSION_LIFETIME'] = datetime.timedelta(seconds=2) # Устанавливаем короткое время жизни

    login_resp = client.post('/login', json={'username': test_user.username, 'password': 'password'})
    ...
    with client.session_transaction() as sess:
        ...
        old_time = (login_time - datetime.timedelta(seconds=3)).replace(tzinfo=datetime.timezone.utc)
